...
/// <summary>
/// Тесты для <see cref="MonitorService"/> — подсчёт простых чисел с синхронизацией через Monitor (lock).
/// </summary>
[TestFixture]
public sealed class MonitorServiceTests
{
    private const string ExpectedVersionName = "Monitor";

    /// <summary>
    /// Проверяет, что <see cref="MonitorService.GetVersionName"/> возвращает ожидаемое имя версии.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// Проверяет, что <see cref="MonitorService.CountPrimes"/> выбрасывает <see cref="ArgumentOutOfRangeException"/>,
    ...
    /// <summary>
    /// Проверяет, что <see cref="MonitorService.CountPrimes"/> выбрасывает <see cref="ArgumentException"/>,
    ...
    /// <summary>
    /// Проверяет, что повторный вызов <see cref="MonitorService.CountPrimes"/> даёт тот же набор простых для тех же параметров.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// Проверяет, что вызов <see cref="MonitorService.CountPrimes"/> при корректных параметрах завершается без исключений.
    /// </summary>
    [Test]

...
/// <summary>
/// Тесты для <see cref="MutexService"/> — подсчёт простых чисел с синхронизацией через Mutex.
/// </summary>
[TestFixture]
public sealed class MutexServiceTests
{
    private const string ExpectedVersionName = "Mutex";

    /// <summary>
    /// Проверяет, что <see cref="MutexService.GetVersionName"/> возвращает ожидаемое имя версии.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// Проверяет, что <see cref="MutexService.CountPrimes"/> выбрасывает <see cref="ArgumentOutOfRangeException"/>,
    ...
    /// <summary>
    /// Проверяет, что <see cref="MutexService.CountPrimes"/> выбрасывает <see cref="ArgumentException"/>,
    ...
    /// <summary>
    /// Проверяет, что повторный вызов <see cref="MutexService.CountPrimes"/> даёт тот же набор простых для тех же параметров.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// Проверяет, что вызов <see cref="MutexService.CountPrimes"/> при корректных параметрах завершается без исключений.
    /// </summary>
    [Test]
